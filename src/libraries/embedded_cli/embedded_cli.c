...
uint8_t embeddedCliEnCmd(EmbeddedCli *cli, uint16_t idx, uint8_t en, uint8_t hd) {
    PREPARE_IMPL(cli);
    
    /* +1 to take into account the help cmd */
    if ((idx + 1) >= impl->bindingsCount)
        return false;
    
    impl->bindings[idx + 1].enabled = en;
    impl->bindings[idx + 1].hidden = hd;
