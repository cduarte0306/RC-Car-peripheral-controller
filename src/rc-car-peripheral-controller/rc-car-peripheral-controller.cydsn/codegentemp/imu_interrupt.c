...
#if !defined(imu_interrupt__REMOVED) /* Check for removal by optimization */
...
void imu_interrupt_Start(void)
{
    ...
    /* Set the priority. */
    imu_interrupt_SetPriority((uint8)imu_interrupt_INTC_PRIOR_NUMBER);
...
{
    #ifdef imu_interrupt_INTERRUPT_INTERRUPT_CALLBACK
        imu_interrupt_Interrupt_InterruptCallback();
    #endif /* imu_interrupt_INTERRUPT_INTERRUPT_CALLBACK */ 
...
*   before the component has been started use imu_interrupt_StartEx instead.
* 
*   When defining ISR functions, the CY_ISR and CY_ISR_PROTO macros should be 
*   used to provide consistent definition across compilers:
*
...
void imu_interrupt_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)imu_interrupt__INTC_NUMBER] = address;
}
...
cyisraddress imu_interrupt_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)imu_interrupt__INTC_NUMBER];
}
...
*
*   Note calling imu_interrupt_Start or imu_interrupt_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after imu_interrupt_Start or imu_interrupt_StartEx has been called. 
...
uint8 imu_interrupt_GetState(void)
{
    ...
    return ((*imu_interrupt_INTC_SET_EN & (uint32)imu_interrupt__INTC_MASK) != 0u) ? 1u:0u;
}
